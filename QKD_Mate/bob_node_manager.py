#!/usr/bin/env python3
"""
Gestore del nodo Bob - Script per il dispositivo dedicato a Bob
"""
import sys
import time
import os
...
class BobNodeManager:
    def __init__(self):
        self.node_name = "BOB"
        self.check_certificates()
        
    def check_certificates(self):
        """Verifica che i certificati necessari per Bob siano presenti"""
        required_certs = ["ca.crt", "client_Bob2.crt", "client_Bob2.key"]
        missing = []
        ...
        key_path = "certs/client_Bob2.key"
    ...
    def check_status(self):
        """Controlla lo stato del nodo Bob"""
        try:
            ...
            if "certificate" in str(e).lower():
                ...
                print("  - Verifica che l'IP 78.40.171.144:443 sia raggiungibile")
                
            return False, None
    
    def get_keys(self, count=1):
        """Richiede chiavi quantistiche dal nodo Bob"""
        try:
    ...
    def continuous_monitor(self, interval=30):
        """Monitoraggio continuo del nodo Bob"""
...
def main():
    ...
    print(f"{BLUE}║{RESET} Gestore Nodo Quantistico BOB {BLUE}║{RESET}")
    print(f"{BLUE}╚{'═'*50}╝{RESET}\n")
    
    manager = BobNodeManager()
    ...
    if len(sys.argv) > 1:
        ...
        if command == "status":
            ...
            print("\nUso:")
            print("  python bob_node_manager.py status     # Controlla stato")
            print("  python bob_node_manager.py monitor [intervallo]  # Monitoraggio continuo")
            print("  python bob_node_manager.py keys [numero]  # Richiedi chiavi")
            
    else:
        # Modalità interattiva
        while True:
            print("\n" + "="*50)
            print("MENU PRINCIPALE - Nodo BOB")
            print("="*50)
            ...
            if choice == "1":
                ...
                print(f"\n{YELLOW}Uscita dal gestore nodo BOB.{RESET}")
                break
